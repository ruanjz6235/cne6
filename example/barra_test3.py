...
class RealBarraData:
    ...
    def setup_class(cls):
        ...
        for date in pd.date_range('2009-12-31', '2026-12-31', freq='YE'):
    ...
    def calculate_single_date_returns(self):
        """测试单日因子收益率计算"""
        print("\n[测试 17] 测试单日因子收益率计算")

        dates = self.factor_df['end_date'].unique()
        if len(dates) == 0:
            print("[SKIP] 无可用日期")
            return
        ...
        result = calculate_constrained_weighted_factor_returns(
            self.factor_df,
            test_date0,
            test_date,
            style_factors,
        ...
        print(f"[PASS] 单日因子收益率计算完成: {test_date}")
    ...
    def calculate_all_factor_returns(self):
        ...
        start, _, cal_type = determine_calc_factor_return_range(self.factor_df)
        if not cal_type:
            return
        if start:
            self.factor_df = self.factor_df[self.factor_df['end_date'] >= start]
        calculate_barra_factor_return(
            self.factor_df,
    ...
    def calculate_factor_corr(self):
        ...
        print("\n[测试 20] 测试因子相关系数")
        self.factor_returns = load_factor_returns()
        start = self.factor_returns['end_date'].tolist()[-30]
        factor_return_correlation_ewma_vectorized(
            self.factor_returns,
            start)
    ...
    def calculate_residual_prediction(self):
        ...
        start = self.residual.index.tolist()[-30]
        # residual_volatility_forecast_vectorized(
        #     self.residual,
        #     self.residual.index,
        #     n_jobs=4)
